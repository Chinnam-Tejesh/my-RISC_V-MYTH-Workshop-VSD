...
\TLV
   ...
   |cpu
      @0
         ...
         $valid = $reset ? 1'b0 : $start ? 1'b1 : (>>3$valid == 1'b1) ? 1'b1 : 1'b0;
         
         $imem_rd_en = ! $reset;
         
         $pc[31:0] = ( >>1$reset) ? 32'b0 : >>3$valid_taken_br ? >>3$br_tgt_pc : ( >>3$pc[31:0] + 32'd4);
      ...
      @1
         ...
         //Rf read
         //$rs1_valid = $rf_rd_en1 && !$reset && !$rf_wr_en && $valid;
         //$rs2_valid = $rf_rd_en2 && !$reset && !$rf_wr_en && $valid;
         $rs1_valid = $rs1risb_valid && !$reset && $valid;
         $rs2_valid = $rs2rsb_valid && !$reset && $valid;
         ...
         //mini ALU
         //$result[31:0] = ($is_addi) ? ($src1_value[31:0] + $imm11t0i[11:0]) : ($is_add) ? ($src1_value[31:0] + $src2_value[31:0]) : 32'd0;
         $result[31:0] = ($is_addi) ? ($src1_value[31:0] + { {20{$imm11t0i[11]}}, $imm11t0i[11:0] }) : ($is_add) ? ($src1_value[31:0] + $src2_value[31:0]) : 32'd0;
         //Rf write
         $rf_wr_en = $rd_valid && !$reset && $valid;
         $rd_not_zero = ($rd[4:0] == 5'd0) ? 1'b0 : 1'b1;
         $rf_wr_valid = $rd_valid && $rf_wr_en && $rd_not_zero && $valid;
         ?$rf_wr_valid
         ...
         //computing the branch address
         //$br_tgt_pc = $pc + {$imm12b, $imm11b, $imm10t5b[5:0], $imm4t1b[3:0], 1'b0};
